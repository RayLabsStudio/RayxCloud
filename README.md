...
A native iPhone and iPad app for Xbox Cloud Gaming. Sign in with your Microsoft account, pick a game from your Game Pass library, and play it full screen with a Bluetooth controller.

It exists for one reason: on iOS the only official way to play xCloud is Safari, and Safari keeps its address bar on screen. RayxCloud is a plain native app, so the game gets the whole display.
...
- An Apple developer account. A paid account gives you a one-year certificate. A free account works too but the app has to be reinstalled every 7 days.
...
2. Select the `RayxCloud` target, then the Signing & Capabilities tab.
...
4. Plug in your iPhone or iPad, choose it as the run destination, and press Run.
5. On the device, the first launch will be blocked until you trust your certificate: Settings, General, VPN & Device Management, tap your developer account, tap Trust.
...
1. Tap **Sign in with Microsoft**. A Microsoft page opens inside the app with your device code already filled in. Log in, then tap Done.
...
3. Tap a game. The stream opens full screen in landscape. Tap anywhere to show the close button.
4. Audio plays through the speaker, wired headphones, or Bluetooth audio.
...
The `App/` directory is new and iOS-only: a SwiftUI shell for sign-in, library, and streaming, plus the WebRTC bridge and a video renderer built on AVSampleBufferDisplayLayer. WebRTC itself comes from the prebuilt [stasel/WebRTC](https://github.com/stasel/WebRTC) package.

```
App/            iOS app: SwiftUI views, WebRTC bridge, video renderer
